...
# Run it like this:
# ./dea_analysis_edgeR_LRT.R counts.tsv clusters.tsv spots_meta.tsv comparisons.tsv TRUE
#
#!/usr/bin/env Rscript
...
for (i in 1:nrow(de_tests)) {
  ...
  animal = droplevels(meta_slice$animal)
  cluster = as.character(meta_slice$cluster)
  cluster[cluster %in% c1] = "A"
  cluster[cluster %in% c2] = "B"
  if (length(unique(animal)) > 1) {
      design = model.matrix(~0 + animal + cluster)
  } else {
  ...
  dge_slice = estimateDisp(dge_slice, design, robust=F)
  fit = glmFit(dge_slice, design, robust=F)
  # Do tests
  print("Testing")
  res = glmLRT(fit)
