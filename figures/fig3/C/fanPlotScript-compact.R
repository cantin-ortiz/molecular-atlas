...
ratio.plot <- 0.5
layers.color <- c('#F48D2D', 'deeppink1', '#B79AC8', 'slateblue3','cyan3', 'darkgreen', col.non.selected)
...
df.cl.id.name <- load.df.cl.id.name(cl.id.names.path)
layer.count <- get.layer.count(spots.table[,c('clusters.named', 'acronym')],T)
...
df.col <- df.colors.vivid
df.col$clusters.colors <- gray.color

cl.iso <- rownames(df.col)[startsWith(rownames(df.col), 'Isocortex')]
df.col[cl.iso, 'clusters.colors'] <- layers.color[sapply(cl.iso, function(x){return(which.max(layer.count[x,1:(ncol(layer.count)-1)]))})] 

fname <- 'isocortex-fan.pdf'
...
for(cl in ordered.labels){
  ...
  plot.arc(r, r+2*r/16,theta.cl[1], theta.cl[3],col = df.col[cl,'clusters.colors'], n.points.edge = 10)
